...
def opt_first(prms_initial:list)->tuple:
    ...
    opt.set_min_objective(func.opt_match_mass_and_effspin) # Set Function to Optimize
...
def opt_third_full(prms_initial:list, detail:bool = True)->tuple:
    ...
    opt.set_upper_bounds([20, 175 * lal.MSUN_SI, 1, 1, 1, math.pi, 2 * math.pi, math.pi, math.pi/2]) 
...
def main_Intrinsic(): 
    ...
    print(f"Second Optimization: Spin2z and Precession Parameters")
    ...
    plt.savefig('./Graphics/main2.png', bbox_inches='tight') 
    plt.show() 
...
def main_Full(): 
    ...
    return max_match, Comp_time, optimized_params, optimized_pol


if __name__ == '__main__': 


    Fitting_Factor, Comp_time, optimized_params, optimized_pol = [],[],[],[]
    ...
    if Intrinsic_or_Extrinsic == "Extrinsic":
        for i in range(len(Approximant_opt)): # Obtaining the result for different approximations in the optimizations
            for j in range(len(Info_target)): # Obtaining the result for different target parameters
                ...
                optimized_pol.append(results[3])
